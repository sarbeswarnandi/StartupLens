...
def get_analytics():
    try:
        ...
        for report in reports:
            score = report.get("scores", {}).get("fundability_score", 0)
            name = report.get("startup_data", {}).get("name", "Unknown")
            sector = report.get("startup_data", {}).get("sector", "Unknown")

            startup_names.append(name)
            fundability_scores.append(score)
            sector_count[sector] = sector_count.get(sector, 0) + 1

            if score > highest_score:
                highest_score = score
                highest_startup = name

        return {
            "total_reports": total_reports,
            "average_fundability": round(sum(fundability_scores) / total_reports, 2),
            "top_sector": max(sector_count, key=sector_count.get),
            "highest_startup": highest_startup,
            "highest_score": highest_score,
            "startup_names": startup_names,
            "fundability_scores": fundability_scores,
